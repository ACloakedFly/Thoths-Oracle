...
with open("hex.data", "rb") as data:
    byte_hex = data.read(9)
    hexrgb = open("hex.txt", "w")
    while len(byte_hex) >= 9:
        colors = byte_hex.split(maxsplit=2)
        r = colors[0].decode("utf-8")
        g = colors[1].decode("utf-8")
        b = colors[2].decode("utf-8")
        print(f"#{r}{g}{b}", end=", ")
        hexrgb.write(f"#{r}{g}{b} ")
        byte_hex = data.read(9)

    hexrgb.close()
    #print(f"#{byte_hex:02x}", end=", ")